...
describe('findMuhuratDates', () => {
  it('uses fixtures when TATHAASTU_API_KEY is missing', async () => {
    ...
    expect(result.dates[0]?.date).toBe('2026-08-23');
    expect(result.dates[0]?.score).toBeGreaterThan(result.dates[1]?.score ?? 0);
  });
  ...
  it('falls back to GET /v1/events/find-dates on 402', async () => {
    ...
    expect(result.endpoint).toBe('/events/find-dates');
  ...
  it('scans GET /v1/events/suitability when find and find-dates are 402', async () => {
    ...
    expect(result.endpoint).toBe('/events/suitability');
