...
function jsonFetch(handler: (url: string, init: { method: string; body?: string; headers: Record<string, string> }) => { status: number; body: unknown }): FetchLike {
...
describe('loadBirthChart', () => {
  it('returns the fixture and setup copy when the key is missing', async () => {
    const loaded = await loadBirthChart(ARJUN, { transport: null, language: 'en' });
    expect(loaded.source).toBe('fixture');
    expect(loaded.data.name).toBe('Arjun');
    expect(loaded.data.lagna.sign).toBe(MOCK_BIRTH_CHART.lagna.sign);
    expect(loaded.setup).toMatch(/TATHAASTU_API_KEY/);
  });

  it('falls back to the fixture on 401', async () => {
  ...
  it('uses GET /v1/kundli/teaser when birth-chart is 402', async () => {
    ...
    expect(loaded.data.lagna.sign).toBe('Libra');
...
describe('matchPeople', () => {
  ...
  it('keeps the lite score when the full POST is 402', async () => {
    ...
    expect(loaded.source).toBe('live');
    expect(loaded.data.total).toBe(22);
  });

  it('returns a fixture pair when transport is missing', async () => {
    const loaded = await matchPeople(PRIYA, ARJUN, { transport: null });
    expect(loaded.source).toBe('fixture');
    expect(loaded.data.total).toBe(28);
    expect(loaded.data.max).toBe(36);
    expect(loaded.data.personA).toBe('Priya');
  });
});
