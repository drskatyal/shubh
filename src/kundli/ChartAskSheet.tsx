...
import { privacyLine, remainingLabel, setupCopy } from '../ask/copy';
import { createExpoRecorder, type Recorder } from '../ask/record';
import type { AskVerdict } from '../ask/types';
import type { Language } from '../i18n/strings';
import type { ChartAskSummary } from '../tathaastu/types';
import { askAboutChart } from './askAboutChart';
import { chartAskPrivacy } from './copy';

type Props = {
  visible: boolean;
  armed: boolean;
  onClose: () => void;
  summary: ChartAskSummary | null;
  language: Language;
...
export function ChartAskSheet({
  ...
  const onMic = async () => {
    if (!wallet || !summary || showPaywall || showSetup) return;
    if (phase === 'recording') {
      setPhase('sending');
      try {
    ...
    <Modal visible={visible} animationType="slide" transparent onRequestClose={onClose}>
      <View style={styles.backdrop}>
        <View style={styles.sheet}>
          ...
          <Text style={styles.kicker}>{summary?.name ?? ''}</Text>
          <Text style={styles.title}>{hi ? 'इस कुंडली के बारे में पूछें' : 'Ask about this chart'}</Text>
          ...
          {showSetup ? (
            ...
            <>
              <Pressable
                onPress={onMic}
                disabled={phase === 'sending' || !armed || !summary}
                style={[
                  styles.mic,
              ...
              <Text style={styles.privacy}>{chartAskPrivacy(language)}</Text>
...
const styles = StyleSheet.create({
  ...
  privacy: { color: '#8A7B68', fontSize: 12, lineHeight: 18, marginTop: 4 },
